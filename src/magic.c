...
{ double *p,*p1,*p2,*p3,*p4,xx,xx1,x1,x2,*VSi;
  ...
  { /* open parallel section */
    ...
    for (i=0;i<m;i++) { 
      ...
      VSi = VS + tid * q * q;
      qmgcv_mmult(VSi,V,p,&bt,&ct,&r,&c,&q); /* V'S_i^0.5 result is rank by cS[i] */ 
      for (p2=VSi,j=0;j<*ip;j++)
      for (p1=d;p1<d+rank;p1++,p2++) *p2 /= *p1;   /* D^{-1} V' S_i^0.5 */
      bt=1;ct=0;r= *ip;c= rank;
      qmgcv_mmult(M[i],VSi,U1U1,&bt,&ct,&r,&c,&rank); /* S_i^0.5 V D^{-1} U_1'U_1 */
      bt=ct=0;r=c=rank;
      qmgcv_mmult(K[i],VSi,M[i],&bt,&ct,&r,&c,ip); /* K_i = D^{-1}V'S_iVD^{-1}U_1'U_1 */
      bt=0;ct=1;r=c=rank;
      qmgcv_mmult(M[i],VSi,VSi,&bt,&ct,&r,&c,ip); /* M_i= D^{-1}V'S_iVD^{-1} */
...
{ int *pi,*pivot,q,n,autoinit,ScS,m,mp,i,j,tp,k,use_sd=0,rank,converged,iter=0,ok,*cucS,
  ...
  *nt = 1;
  ...
  /* qmgcv_qr(X,&n,&q,pivot,tau);*/
  qmgcv_pqr(X,&n,&q,pivot,tau,nt);
  ...
  */
  ...
  tp=1;i=1;/*left=1;qmgcv_qrqy(y0,X,tau,&n,&i,&q,&left,&tp);*/ /* first q elements are y1 */
  qmgcv_pqrqy(y0,X,tau,&n,&q,&i,&tp,nt);
  /* form y'y */
  ...
  { Si=array2d(m,q*q);
    i=0;j=1;
    for (p=S,k=0;k<m;p+=cS[k]*q,k++)  qmgcv_mmult(Si[k],p,p,&i,&j,&q,&q,cS+k);   
  }
  ...
  if (L_exists) {
    i=0;j=1;if (mp) qmgcv_mmult(sp,L,sp0,&i,&i,&m,&j,&mp); /* form sp = L sp0  */
...
/*  for (i=0;i<m;i++) Rprintf("%g  ",exp(sp[i]));Rprintf("\n");*/
  ...
  { magic_gH(U1U1,M,K,VS,My,Ky,yK,hess,grad1,dnorm,ddelta,sp,d2norm,d2delta,S,
    ...
    if (L_exists) { /* transform to grad w.r.t. sp0 */
      i=0;j=1;qmgcv_mmult(grad,L,grad1,&j,&i,&mp,&j,&m);
    } else {
    ...
    if (L_exists) {
      i=0;j=1;qmgcv_mmult(sp,L,sp0,&i,&i,&m,&j,&mp); /* form sp = L sp0 */
      for (p=sp,p1=lsp0,p2=sp+m;p<p2;p++,p1++) *p += *p1; /* form sp= L sp0 + lsp0 */
    } else { /* sp0 and sp are identical */
      for (i=0;i<m;i++) sp[i]=sp0[i];
    }
  ...
  { converged=0;iter=0;
    ...
    { iter++;
      ...
      { try++; if (try==4&&!use_sd) {use_sd=1;step=sd_step;}
        ...
        if (L_exists) {
          i=0;j=1;qmgcv_mmult(sp,L,nsp,&i,&i,&m,&j,&mp); /* form sp = L nsp */
	...
	if (L_exists) { /* transform grad and hess */
          i=0;j=1;qmgcv_mmult(grad,L,grad1,&j,&i,&mp,&j,&m);
          qmgcv_mmult(u0,L,u,&j,&i,&mp,&m,&m); /* u0 contains L'H */
          qmgcv_mmult(u,u0,L,&i,&i,&mp,&mp,&m); /* u contains L'HL - the transformed hessian */
        } else {
          p = grad;grad=grad1;grad1=p;
        }
               
        qmgcv_symeig(u,ev,&mp,&use_dsyevd,&TRUE,&FALSE); /* columns of hess are now eigen-vectors */
    ...
    if (L_exists) {
      i=0;j=1; qmgcv_mmult(sp,L,sp0,&i,&i,&m,&j,&mp); /* form sp = L nsp */
      for (p=sp,p1=lsp0,p2=sp+m;p<p2;p++,p1++) *p += *p1; /* form sp= L sp0 + lsp0 */
    } else { /* nsp and sp are identical */
      for (i=0;i<m;i++) sp[i]=sp0[i];
    }
