/* Convenient C wrappers for calling LAPACK and LINPACK + other matrix routines using same 
   ...
   * qmgcv_pqr - parallel QR based on row blocking ok for few cores and n>>p but
   ...
   * qmgcv_piqr - pivoted QR that simply parallelizes the 'householder-to-unfinished-cols'
                 step. Storage exactly as standard LAPACK pivoted QR.
   * qmgcv_Rpiqr - wrapper for above for use via .call 
   * qmgcv_pmmult - parallel matrix multiplication.
   * qmgcv_Rpbsi - parallel inversion of upper triangular matrix.
...
#ifdef _OPENMP // needs to precede R.h (and qmgcv.h)
#include <omp.h>
#endif
...
/* routine to facilitate reading dumped matrices back into R - debugging use only
   ...
   oo <- .C("read_mat",as.double(0),r=as.integer(0),c=as.integer(0),
             as.character("/home/sw283/tmp/badmat.dat"),PACKAGE="qmgcv")
...
void qmgcv_tensor_mm(double *X,double *T,int *d,int *m,int *n) {
...
SEXP qmgcv_tmm(SEXP x,SEXP t,SEXP D,SEXP M, SEXP N) {
  /* wrapper for calling qmgcv_tensor_mm using .Call */
  double *X,*T;
  int *d,*m,*n;
  X = REAL(x);T=REAL(t);
  d = INTEGER(D);
  m = INTEGER(M);
  n = INTEGER(N);
  qmgcv_tensor_mm(X,T,d,m,n);
  return(R_NilValue);
}

void qmgcv_mmult0(double *A,double *B,double *C,int *bt,int *ct,int *r,int *c,int *n)
/* This code doesn't rely on the BLAS...
   ...
   system.time(
   A<-matrix(.C("qmgcv_mmult",as.double(A),as.double(B),as.double(C),as.integer(1),as.integer(1),
   ...
   system.time(
   A<-matrix(.C("qmgcv_mmult",as.double(A),as.double(B),as.double(C),as.integer(1),as.integer(0),
          as.integer(r),as.integer(c),as.integer(n))[[1]],r,c))
   range(as.numeric(t(B)%*%C-A))
   A<-matrix(0,r,c);B<-matrix(rnorm(r*n),r,n);C<-matrix(rnorm(n*c),c,n)
   system.time(
   A<-matrix(.C("qmgcv_mmult",as.double(A),as.double(B),as.double(C),as.integer(0),as.integer(1),
          as.integer(r),as.integer(c),as.integer(n))[[1]],r,c))
   range(as.numeric(B%*%t(C)-A))
   A<-matrix(0,r,c);B<-matrix(rnorm(r*n),r,n);C<-matrix(rnorm(n*c),n,c)
   system.time(
   A<-matrix(.C("qmgcv_mmult",as.double(A),as.double(B),as.double(C),as.integer(0),as.integer(0),
          as.integer(r),as.integer(c),as.integer(n))[[1]],r,c))
...
void qmgcv_mmult(double *A,double *B,double *C,int *bt,int *ct,int *r,int *c,int *n) {
...
SEXP qmgcv_madi(SEXP a, SEXP b,SEXP ind,SEXP diag) {
/* Performs 
...
SEXP qmgcv_pmmult2(SEXP b, SEXP c,SEXP bt,SEXP ct, SEXP nthreads) {
/* parallel matrix multiplication using .Call interface */
  ...
  #endif
  qmgcv_pmmult(A,B,C,&Bt,&Ct,&r,&col,&n,&nt);
  UNPROTECT(1);
  return(a);
} /* qmgcv_pmmult2 */

int qmgcv_bchol0(double *A,int *piv,int *n,int *nt,int *nb) {
...
int qmgcv_bchol(double *A,int *piv,int *n,int *nt,int *nb) {
/* Lucas (2004) "LAPACK-Style Codes for Level 2 and 3 Pivoted Cholesky Factorizations" 
   block pivoted Choleski algorithm 5.1. Note some misprints in paper, noted below. 
   nb is block size, nt is number of threads, A is symmetric
   +ve semi definite matrix and piv is pivot sequence. 
...
int qmgcv_pchol(double *A,int *piv,int *n,int *nt) {
...
SEXP qmgcv_Rpchol(SEXP Amat,SEXP PIV,SEXP NT,SEXP NB) {
/* routine to Choleski decompose n by n  matrix Amat with pivoting 
   using routine qmgcv_bchol to do the work. Uses NT parallel
...
*/
  ...
  A = REAL(Amat);
  piv = INTEGER(PIV);
  // r = qmgcv_pchol(A,piv,&n,&nt); 
  r = qmgcv_bchol(A,piv,&n,&nt,&nb);
...
int qmgcv_piqr(double *x,int n, int p, double *beta, int *piv, int nt) {
...
SEXP qmgcv_Rpiqr(SEXP X, SEXP BETA,SEXP PIV,SEXP NT, SEXP NB) {
...
void qmgcv_pmmult(double *A,double *B,double *C,int *bt,int *ct,int *r,int *c,int *n,int *nt) {
  /* 
  ...
  *nt = 1;
  #endif
  if (*nt == 1) {
    qmgcv_mmult(A,B,C,bt,ct,r,c,n); /* use single thread version */
    return;
  }
  if (*bt) { /* so B is n by r */
    transa = 'T';  
...
SEXP qmgcv_Rpcross(SEXP A, SEXP NT,SEXP NB) {
...
SEXP qmgcv_chol_down(SEXP r,SEXP ru,SEXP N,SEXP K, SEXP UT) {
...
SEXP qmgcv_chol_up(SEXP r,SEXP U,SEXP N,SEXP UP,SEXP EPS) {
/* wrapper for calling chol_down using .Call */
  double *R,*u,*eps;
  int *n,*up;
  R = REAL(r);u=REAL(U);
...
void qmgcv_chol(double *a,int *pivot,int *n,int *rank)
/* a stored in column order, this routine finds the pivoted choleski decomposition of matrix a 
   library(qmgcv)
   X<-matrix(rnorm(16),4,4);D<-X%*%t(X)
   rank<-0
   er<-.C("qmgcv_chol",as.double(D),as.integer(rep(0,4)),as.integer(4),as.integer(rank))
...
void qmgcv_svd_full(double *x,double *vt,double *d,int *r,int *c)
...
um<-.C("qmgcv_svd_full",as.double(X),double(q*q),double(q),as.integer(n),as.integer(q),
        PACKAGE="qmgcv")
er<-La.svd(X)
...
void qmgcv_td_qy(double *S,double *tau,int *m,int *n, double *B,int *left,int *transpose)
/* Multiplies m by n matrix B by orthogonal matrix returned from qmgcv_tri_diag and stored in 
...
void qmgcv_tri_diag(double *S,int *n,double *tau)
...
void qmgcv_pbsi0(double *R, int *n,int *nt) {
...
void qmgcv_pbsi1(double *R, int *n,int *nt) {
/* Form inverse of upper triangular R (n by n) in situ.
   Block oriented, using BLAS/LAPACK calls. 
*/
...
void qmgcv_pbsi(double *R,int *r,int *nt) {
...
SEXP qmgcv_Rpbsi(SEXP A, SEXP NT) {
/* parallel back sub inversion of upper triangular matrix A 
   designed for use with .call, but using qmgcv_pbsi to do the work. 
*/
  int nt,r;
  double *R;
  nt = asInteger(NT);
...
void qmgcv_PPt1(double *A,double *R,int *r,int *nt) {
/* Computes A=RR', where R is r by r upper triangular. BLAS version.
   ...
   library(qmgcv);R <- matrix(0,n,n);R[ii] <- runif(n*(n+1)/2)
   system.time(A <- qmgcv:::pRRt(R,2))
...
void qmgcv_PPt(double *A,double *R,int *r,int *nt) {
...
SEXP qmgcv_RPPt(SEXP a,SEXP r, SEXP NT) {
/* Form a = rr' where r is upper triangular. 
   designed for use with .Call, but using qmgcv_PPt to do the work. 
*/
  int nt,n;
  double *R,*A;
  nt = asInteger(NT);
...
void qmgcv_backsolve0(double *R,int *r,int *c,double *B,double *C, int *bc) 
/* BLAS free version
   ...
   R facilitates use with output from qmgcv_qr). This is just a standard back 
   substitution loop.
*/  
{ int i,j,k;
  double x,*pR,*pC;
...
void qmgcv_forwardsolve0(double *R,int *r,int *c,double *B,double *C, int *bc) 
/* BLAS free version
   Finds C = R^{-T} B where R is the c by c matrix stored in the upper triangle 
   of r by c argument R. B is c by bc. (Possibility of non square argument
   R facilitates use with output from qmgcv_qr). This is just a standard forward 
   substitution loop.
*/  
{ int i,j,k;
  double x;
...
void qmgcv_backsolve(double *R,int *r,int *c,double *B,double *C, int *bc,int *right) 
/* BLAS version
   If *right==0:
   Finds C = R^{-1} B where R is the c by c matrix stored in the upper triangle 
   of r by c argument R. B is c by bc. (Possibility of non square argument
...
{ double *pR,*pC,alpha=1.0;
 ...
 void qmgcv_forwardsolve(double *R,int *r,int *c,double *B,double *C, int *bc,int *right) 
/* BLAS version
   If *right == 0:
   Finds C = R^{-T} B where R is the c by c matrix stored in the upper triangle 
   of r by c argument R. B is c by bc. (Possibility of non square argument
...
void qmgcv_pforwardsolve(double *R,int *r,int *c,double *B,double *C, int *bc,int *nt) 
...
void qmgcv_pbacksolve(double *R,int *r,int *c,double *B,double *C, int *bc,int *nt) 
/* parallel back solve, using nt threads.
   Finds C = R^{-T} B where R is the c by c matrix stored in the upper triangle 
   of r by c argument R. B is c by bc. (Possibility of non square argument
   R facilitates use with output from qmgcv_qr). 
*/  
{ double *pR,*pC,alpha=1.0;
  int cpt,cpf,nth,i,cp;
  char side='L',uplo='U',transa='N',diag='N';
...
SEXP qmgcv_Rpforwardsolve(SEXP R, SEXP B,SEXP NT) {
/* .Call wrapper for qmgcv_pforwardsolve.
   Return object a matrix.
...
*/
  ...
  qmgcv_pforwardsolve(R0,&r,&c,b,C0,&bc,&nt);
   
  UNPROTECT(1);
  return(C);
} /* qmgcv_Rpforwardsolve */

SEXP qmgcv_Rpbacksolve(SEXP R, SEXP B,SEXP NT) {
/* .Call wrapper for qmgcv_pbacksolve.
   Return object a matrix.
...
*/
  ...
  qmgcv_pbacksolve(R0,&r,&c,b,C0,&bc,&nt);
...
/* x contains qr decomposition of r by c matrix as computed by qmgcv_pqr 
...
/* x contains qr decomposition of r by c matrix as computed by qmgcv_pqr 
   ...
   This version matches qmgcv_pqrqy0, which is inferior to current code.
*/
  int i,j,k,n;
  double *Rs;
...
void qmgcv_pqrqy0(double *b,double *a,double *tau,int *r,int *c,int *cb,int *tp,int *nt) {
/* Applies factor Q of a QR factor computed in parallel to b. 
   ...
   a and tau are the result of qmgcv_pqr  

   This version matches qmgcv_pqr0, which scales less well than current code. 
*/
  int i,j,k,l,left=1,n,nb,nbf,nq,TRUE=1,FALSE=0;
  double *x0,*x1,*Qb;  
  #ifdef OMP_REPORT
  Rprintf("qmgcv_pqrqy0...");
  #endif
  ...
  if (k==1) { /* single block case */ 
    ...
    qmgcv_qrqy(b,a,tau,r,cb,c,&left,tp);
  ...
  if (*tp) { /* Q'b */
    ...
    { /* open parallel section */
      ...
      for (i=0;i<k;i++) {
        ...
        qmgcv_qrqy(x1,a + i * nb * *c,tau + i * *c,&n,cb,c,&left,tp);
    ...
    qmgcv_qrqy(Qb,a + *r * *c,tau + k * *c,&nq,cb,c,&left,tp);
...
void qmgcv_pqrqy(double *b,double *a,double *tau,int *r,int *c,int *cb,int *tp,int *nt) {
/* Applies factor Q of a QR factor computed by qmgcv_pqr to b. 
   b is physically r by cb, but if tp = 0 it contains a c by cb matrix on entry, while if
   tp=1 it contains a c by cb matrix on exit. Unused elments of b on entry assumed 0. 
   a and tau are the result of qmgcv_pqr.

   Note that in multi-threaded mode this uses qmgcv_pqr0, which is thread safe, but level 2.
   qmgcv_pqr is level 3 but not thread safe.  qmgcv_pqrqy itself is not thread safe - i.e. 
...
*/
  ...
  if (*cb==1 || *nt==1) qmgcv_qrqy(b,a,tau,r,cb,c,&left,tp);
    ...
    { /* open parallel section */
      ...
      for (i=0;i<nth;i++) {
        ...
        qmgcv_qrqy0(b + j * *r,a,tau,r,&ki,c,&left,tp);
      }
    } /* end parallel section */
...
void qmgcv_pqr(double *x,int *r, int *c,int *pivot, double *tau, int *nt) {
/* Function called from qmgcv to request a multi-threaded pivoted QR decomposition.
...
*/
  //Rprintf("pqr %d ",*nt);
  if (*nt==1) qmgcv_qr(x,r,c,pivot,tau); else { /* call bpqr */ 
    /* int bpqr(double *A,int n,int p,double *tau,int *piv,int nb,int nt)*/
...
void qmgcv_pqr0(double *x,int *r, int *c,int *pivot, double *tau, int *nt) {
/* parallel qr decomposition using up to nt threads. 
   ...
   * pivot is a c vector and tau is a (nt+1)*c vector.
   ...
   for use by getRpqr and qmgcv_pqrqy
...
*/
  ...
  if (k==1) qmgcv_qr(x,r,c,pivot,tau); else { /* multi-threaded version */
    ...
    { /* open parallel section */
      ...
      for (i=0;i<k;i++) { /* QR the blocks */
        ...
        qmgcv_qr(xi,&n,c,piv + i * *c,tau + i * *c);
    ...
    FREE(piv);
    n = k * *c;
    qmgcv_qr(R,&n,c,pivot,tau + k * *c); /* final pivoted QR */
  } 
  #ifdef OMP_REPORT
  Rprintf("done\n");
  #endif
...
void qmgcv_qr(double *x, int *r, int *c,int *pivot,double *tau)
/* call LA_PACK to get pivoted QR decomposition of x
   ...
   um<-.C("qmgcv_qr",as.double(X),as.integer(r),as.integer(c),as.integer(pivot),as.double(tau))
...
void qmgcv_qr2(double *x, int *r, int *c,int *pivot,double *tau)
...
void qmgcv_qrqy0(double *b,double *a,double *tau,int *r,int *c,int *k,int *left,int *tp) {
/* qmgcv_qrqy is not thread safe, because of the behaviour of dormqr (and similar functions).
   ...
   Information about Q has been returned from qmgcv_qr, and is stored in tau and 
...
void qmgcv_qrqy(double *b,double *a,double *tau,int *r,int *c,int *k,int *left,int *tp)
/* applies k reflectors of Q of a QR decomposition to r by c matrix b.
   Apply Q from left if left!=0, right otherwise.
   Transpose Q only if tp!=0.
   Information about Q has been returned from qmgcv_qr, and is stored in tau and 
   ...
   qrx<-qr(X)
   pivot<-rep(1,c);tau<-rep(0,c)
   um<-.C("qmgcv_qr",a=as.double(X),as.integer(r),as.integer(c),as.integer(pivot),tau=as.double(tau))
   y<-1:4;left<-1;tp<-0;cy<-1
   er<-.C("qmgcv_qrqy",as.double(y),as.double(um$a),as.double(um$tau),as.integer(r),as.integer(cy),as.integer(c),
        as.integer(left),as.integer(tp),PACKAGE="qmgcv")
   er[[1]];qr.qy(qrx,y)
...
um<-.C("update_qr",as.double(Q),as.double(R),as.integer(n),as.integer(q),
        as.double(lam),as.integer(q-1),PACKAGE="qmgcv")
...
void qmgcv_symeig(double *A,double *ev,int *n,int *use_dsyevd,int *get_vectors,
                 int *descending)
...
/* gets eigenvalues and (optionally) vectors of symmetric matrix A. 
   ...
   ******************************************************
   ...
   um<-.C("qmgcv_symeig",as.double(A),as.double(d),as.integer(ncol(A)),
           as.integer(0),as.integer(1),as.integer(1),PACKAGE="qmgcv")
   U <- matrix(um[[1]],n,n)
   er$vectors;U
   er$values;um[[2]]
*/  
...
void qmgcv_trisymeig(double *d,double *g,double *v,int *n,int *getvec,int *descending) 
  ...
  *nt = 1; /* reset number of threads to 1 if openMP not available  */ 
  ...
  for (j=0;j< *n;j++) {
    ...
    { for (i=0;i<j+1;i++) d[i]=a[i]; /* copy leading diagonal of T_j */
      ...
      kk = j + 1;
      qmgcv_trisymeig(d,g,v,&kk,&one,&one);
