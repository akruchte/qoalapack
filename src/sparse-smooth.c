...
/* Create a kd tree and return a handle for it to R as attribute 
   ...
   nei <- qmgcv:::kd.nearest(kd,X,x,1)
   bb <- qmgcv:::kd.radius(kd,X,x,.01) 
...
/* NOTE: n modified on exit!!
   ...
   Some R test code...
   cd ~simon/qmgcv-related/sparse-smooth
   R CMD SHLIB kd-tree.c  
   R 
...
*/
  ...
  for (j=0;j<*n;j++) { /* work through all points */
    ...
    /* call qmgcv_svd_full to pseudoinvert M */
    i = 6;
    qmgcv_svd_full(M,Vt,sv,&kk,&i); 
    ...
    i=6;
    qmgcv_mmult(Mi,Vt,M,&True,&True,&i,&k,&i);
