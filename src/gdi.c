...
{ int i,off,nc,bt,ct;
  ...
  nc = rSncol[k];
  bt=1;ct=0;
  qmgcv_mmult(work,rSk,x,&bt,&ct,&nc,xcol,q);
  bt=0;
  qmgcv_mmult(y,rSk,work,&bt,&ct,q,xcol,&nc);
}
...
{ double *Sb,*Skb,*work,*work1,*p1,*p0,*p2,xx;
  ...
  bt=0;ct=0;qmgcv_mmult(work,E,beta,&bt,&ct,Enrow,&one,q);
  bt=1;ct=0;qmgcv_mmult(Sb,E,work,&bt,&ct,q,&one,Enrow); /* S \hat \beta */
  ...
  for (p1=Skb,rSoff=0,i=0;i<*M;i++) { /* first part of first derivatives */
     /* form S_k \beta * sp[k]... */
     bt=1;ct=0;qmgcv_mmult(work,rS + rSoff ,beta,&bt,&ct,rSncol+i,&one,q);
     for (j=0;j<rSncol[i];j++) work[j] *= sp[i]; 
     bt=0;ct=0;qmgcv_mmult(p1,rS + rSoff ,work,&bt,&ct,q,&one,rSncol+i);
     rSoff += *q * rSncol[i];
  ...
  if (*deriv>1)  for (m=0;m < Mtot;m++) { /* Hessian */
     bt=0;ct=0;qmgcv_mmult(work1,E,b1 + m * *q,&bt,&ct,Enrow,&one,q);
     bt=1;ct=0;qmgcv_mmult(work,E,work1,&bt,&ct,q,&one,Enrow);  /* S dbeta/drho_m */
  ...
  bt=1;ct=0;qmgcv_mmult(work,b1,Sb,&bt,&ct,&Mtot,&one,q);
...
{ double *tau,ldet,*p,*Qt;
  ...
  qmgcv_qr(X,r,r,pivot,tau); /* get QR=X itself */
  ...
  /* Now get the inverse of X. X^{-1} = R^{-1}Q' */
    ...
    qmgcv_qrqy(Qt,X,tau,r,r,r,&TRUE,&TRUE); /* Extracting the orthogonal factor Q' */

    qmgcv_backsolve(X,r,r,Qt,Xi,r,&FALSE); /* Now Xi contains the row pivoted inverse of X */
...
{ double *R,*work,*tau,*rS1,*rS2, *S,*Si,*Sb,*B,*Sg,*p,*p1,*p2,*p3,*p4,*frob,max_frob,x,*spf,Rcond;
  ...
  */
  ...
  for (rSoff=i=0;i<Mf;p+= *q * *q,rSoff+=rSncol[i],i++) {
    bt=0;ct=1;qmgcv_mmult(p,sqrtS+rSoff * *q,sqrtS+rSoff * *q,&bt,&ct,q,q,rSncol+i);   
  }
  ...
  */
    ...
    qmgcv_qr(Sb, &Q, q ,pivot,tau); /* obtain pivoted QR decomposition of Sb */
  ...
  /* Form Q'Sg... */ 
    left=1;tp=1; 
    qmgcv_qrqy(Sg,Sb,tau,&Q,q,&Q,&left,&tp);
  ...
  /* transform remaining S_i in gamma1 */ 
    for (p1=p=Si,i=0;i<Mf;i++,p += *q * Q,p1 += *q *Qr) if (gamma1[i]) {
      left=1;tp=1;
      qmgcv_qrqy(p,Sb,tau,&Q,q,&Q,&left,&tp); /* Q'Si */
      p2=p+r;p3=p1;
  ...
  /* Transform the square roots of Si */
    if (*deriv) { /* transformed rS1 only needed for derivatives */
      ...
      left=1;tp=1;
      qmgcv_qrqy(rS2,Sb,tau,&Q,&tot_col,&Q,&left,&tp); /* Q'rS2 */
  ...
  *det = qr_ldet_inv(R,q,B,deriv); /* R=S' here */
  ...
  if (*deriv) { /* get the first derivatives */
    ...
    bt=0;ct=0;qmgcv_mmult(Si,B,sqrtS,&bt,&ct,q,&tot_col,q);
  ...
  if (*deriv==2) { /* get second derivatives, as well */
    ...
    for (p1=Si,p=rS2,p2=rS1,i=0;i<*M;p2+= *q * rSncol[i], p += *q *rSncol[i],i++,p1 += *q * *q) {
      bt=0;ct=1;qmgcv_mmult(p1,p,p2,&bt,&ct,q,q,rSncol+i);
    }
...
{ double *rS, *Un, *U, *Si,*Sb,*B,*C,*Sg,*p,*p1,*p2,*p3,*frob,*ev,max_frob,x,*spf;
  ...
  /* Form the scaled sum of the Si in alpha and eigen-decompose it to get its rank */
    if (n_gamma1) { /* stuff left in gamma1, so have to work out rank of contents of alpha */
      ...
      qmgcv_symeig(Sb,ev,&Q,&FALSE,&FALSE,&FALSE); /* get eigenvalues (ascending) of scaled sum over alpha */
  ...
  /* Form the dominant term and eigen-decompose it */
    ...
    qmgcv_symeig(Sb,ev,&Q,&FALSE,&TRUE,&TRUE); /* get eigen decomposition of dominant term (ev descending) */
    
  /* .... U points to Sb, which now contains eigenvectors */
    if (iter==1) for (p=U,p1=Qf,p2 = p+Q*Q;p<p2;p++,p1++) *p1 = *p;
    else { 
      bt=0;ct=0;qmgcv_mmult(B,Qf+K * *q,U,&bt,&ct,q,&Q,&Q);
  ...
  /* Form S' the similarity transformed S */
    if (K>0) { /* deal with upper right component B */
      ...
      bt=0;ct=0;qmgcv_mmult(B,C,U,&bt,&ct,&K,&Q,&Q);
      /* Replace B into S */
    ...
    bt=0;ct=0;qmgcv_mmult(B,Sg,U,&bt,&ct,&Q,&Q,&Q); /* SgU is in B */
    bt=1;ct=0;qmgcv_mmult(C,U,B,&bt,&ct,&Q,&Q,&Q);  /* U'SgU is in C */ 
  ...
  /* Transform the Si in gamma' */
    ...
    for (p1=p=Si,i=0;i<Mf;i++,p += Q*Q,p1 +=Qr*Qr) if (gamma1[i]) { /* p points to old Si, and p1 to new */
      bt=1;ct=0;qmgcv_mmult(B,Un,p,&bt,&ct,&Qr,&Q,&Q);
      bt=0;ct=0;qmgcv_mmult(p1,B,Un,&bt,&ct,&Qr,&Qr,&Q); 
    }
  /* Update K, Q and gamma */   
    K = K + r; Q = Qr;
    for (i=0;i<Mf;i++) gamma[i] = gamma1[i];
  ...
  *det = qr_ldet_inv(C,q,B,deriv);
  ...
  if (*deriv==2) { /* get second derivatives, as well */
    for (p1=Si,p=rS,i=0;i<*M;p += *q *rSncol[i],i++,p1 += *q * *q) { /* loop through creating S^{-1} S_i and storing in Si*/
      bt=0;ct=0;qmgcv_mmult(C,B,p,&bt,&ct,q,rSncol+i,q);
      bt=0;ct=1;qmgcv_mmult(p1,C,p,&bt,&ct,q,q,rSncol+i);
    }
    for (i=0;i<*M;i++) for (j=i;j<*M;j++) 
      det2[i + *M * j] = det2[j + *M * i] = -sp[i]*sp[j]*trAB(Si + *q * *q *i,Si + *q * *q *j,q,q);
    for (i=0;i<*M;i++) det2[i + *M * i] += det1[i];
...
{ double *diagKKt,xx,*KtTK,*PtrSm,*PtSP,*trPtSP,*work,*pdKK,*p1,*pTkm;
  ...
  bt=1;ct=0;qmgcv_mmult(det1,Tk,diagKKt,&bt,&ct,&Mtot,&one,n); /* tr(TkKK') */ 
  ...
  { /* parallel section start */
    ...
    for (m=0;m < *M;m++) { /* loop through penalty matrices */
      ...
      tid = omp_get_thread_num(); /* thread running this bit */
      #endif    
      bt=1;ct=0;qmgcv_mmult(PtrSm + tid * *r * max_col,P,rS+rSoff[m] * *q,&bt,&ct,r,rSncol+m,q);
      /*rSoff += rSncol[m];*/
      ...
      if (deriv2) { /* get P'S_mP */
        bt=0;ct=1;qmgcv_mmult(PtSP+ m * *r * *r,PtrSm + tid * *r * max_col,
...
{ double *diagKKt,*diagKKtKKt,xx,*KtTK,*KtTKKtK,*KKtK,*KtK,*work,*pTk,*pTm,*pdKKt,*pdKKtKKt,*p0,*p1,*p2,*p3,*pd,
  ...
  *trA = diagABt(diagKKt,K,K,n,r); 
  ...
  bt=1;ct=0;qmgcv_pmmult(KtK,K,IpK,&bt,&ct,r,r,n,nt);  
  if (neg_w) FREE(IpK); else FREE(IpK);
  KKtK = (double *)CALLOC((size_t)*n * *r,sizeof(double));
  bt=0;ct=0;qmgcv_pmmult(KKtK,K,KtK,&bt,&ct,n,r,r,nt);  
  ...
  if (deriv2) {
    ...
    { /* open parallel section */
      ...
      for (k=0;k < *M;k++) {
        ...
        bt=ct=0;qmgcv_mmult(KtTKKtK + k * *r * *r ,KtTK + j,KtK,&bt,&ct,r,r,r);
      }
    } /* parallel section end */
  } else { KtTK=KtTKKtK=(double *)NULL;}
  
  /* evaluate first and last terms in first derivative of tr(F) */
  bt=1;ct=0;qmgcv_mmult(trA1,Tk,diagKKt,&bt,&ct,M,&one,n); /* tr(KK'Tk) */ 
  bt=1;ct=0;qmgcv_mmult(work,Tk,diagKKtKKt,&bt,&ct,M,&one,n); /* tr(KK'TkKK') */
  ...
  { /* open parallel section */
    ...
    for (m=0;m < *M;m++) { 
      ...
      tid = omp_get_thread_num(); /* thread running this bit */
      #endif
      bt=1;ct=0;qmgcv_mmult(PtrSm + *r * *q * tid,P,rS+rSoff[m] * *q,&bt,&ct,r,rSncol+m,q);
      bt=0;ct=0;qmgcv_mmult(KPtrSm + *n * *q * tid,K,PtrSm + *r * *q * tid ,&bt,&ct,n,rSncol+m,r); 
      if (deriv2) {
        bt=0;ct=0;qmgcv_mmult(KtKPtrSm + *r * *q * tid,KtK,PtrSm + *r * *q * tid,&bt,&ct,r,rSncol+m,r); 
        bt=0;ct=1;qmgcv_mmult(PtSP+ m * *r * *r,PtrSm + *r * *q * tid,PtrSm + *r * *q * tid,&bt,&ct,r,r,rSncol+m);
    
        bt=0;ct=0;qmgcv_mmult(KKtKPtrSm + *n * *q * tid,KKtK,PtrSm + *r * *q * tid,&bt,&ct,n,rSncol+m,r);      
        bt=0;ct=1;qmgcv_mmult(PtSPKtK + m * *r * *r,PtrSm + *r * *q * tid,KtKPtrSm+ *r * *q * tid,&bt,&ct,r,r,rSncol+m); 
...
{ double *x1;
  ...
  if (neg_w) { /* apply V */
    ...
    if (right) {
      qmgcv_backsolve(R,&nr,&r,x,x1, &c,&right);         /* x1 = x R^{-1} */
      bt=0;ct=1;qmgcv_mmult(y,x1,Vt,&bt,&ct,&c,&r,&r);   /* y = x R^{-1} V */
    } else {
      bt=1;ct=0;qmgcv_mmult(x1,Vt,x,&bt,&ct,&r,&c,&r);   /* x1 = V x */    
      qmgcv_backsolve(R,&nr,&r,x1,y, &c,&right);         /* y = R^{-1} V x */
    }
    FREE(x1);
  } else qmgcv_backsolve(R,&nr,&r,x,y, &c,&right);            /* y = R^{-1} x */
} /* applyP */
...
{ double *x1;
  ...
  if (neg_w) { /* apply V */
    x1 = (double *)CALLOC((size_t)r*c,sizeof(double));
    if (right) {
      bt=0;ct=0;qmgcv_mmult(x1,x,Vt,&bt,&ct,&c,&r,&r);   /* x1 = x V' */
      qmgcv_forwardsolve(R,&nr,&r,x1,y, &c,&right);       /* y = x V' R^{-T} */
    } else {
      qmgcv_forwardsolve(R,&nr,&r,x,x1, &c,&right);      /* x1 = R^{-T} x */
      bt=0;ct=0;qmgcv_mmult(y,Vt,x1,&bt,&ct,&r,&c,&r);   /* y = V'R^{-T} x */    
    }
    FREE(x1);
  } else qmgcv_forwardsolve(R,&nr,&r,x,y, &c,&right);            /* y = R^{-T} x */
} /* applyPt */
...
{ int n_2dCols,i,j,k,one=1,bt,ct;
  ...
  bt=0;ct=0;qmgcv_mmult(eta1,X,b1,&bt,&ct,n,M,r); /* first deriv of eta */
  ...
  if (*deriv2) { /* then second derivatives needed */
    ...
    for (i=0;i<*M;i++) for (k=i;k<*M;k++) { 
      ...
      bt=1;ct=0;qmgcv_mmult(Skb,X,work,&bt,&ct,r,&one,n); /* X'f */
    ...
    bt=0;ct=0;qmgcv_mmult(eta2,X,b2,&bt,&ct,n,&n_2dCols,r); /* second derivatives of eta */
  }
...
{ int n_2dCols,i,j,k,one=1,bt,ct,ntot,kk;
  ...
  for (i=0;i < ntot ;i++) { /* first derivative loop */
    if (i < *n_theta) { /* Db_th is from direct dependence of deviance on theta */
      bt=1;ct=0;qmgcv_mmult(Db_th,X,Det_th + i * *n,&bt,&ct,r,&one,n);
  ...
  bt=0;ct=0;qmgcv_mmult(eta1,X,b1,&bt,&ct,n,&ntot,r); /* first deriv of eta */
  ...
  if (*deriv2) { /* then second derivatives needed */
    ...
    for (i=0;i<ntot;i++) for (k=i;k<ntot;k++) { 
      ...
      bt=1;ct=0;qmgcv_mmult(Db_th,X,work,&bt,&ct,r,&one,n); /* D_bbb^lpq db_q/dtheta_j db_p/dtheta_k */
      /* second term */
      if (k < *n_theta) { 
        p0 = Det2_th + *n * k;
        p1 = eta1 + *n * i; 
        for (j=0;j<*n;j++,p0++,p1++) work[j] = *p0 * *p1 ;
        bt=1;ct=0;qmgcv_mmult(work1,X,work,&bt,&ct,r,&one,n); 
      } else {
      ...
      if (i< *n_theta && k < *n_theta) {
        p0 = Det_th2 + *n * kk;
        bt=1;ct=0;qmgcv_mmult(work,X,p0,&bt,&ct,r,&one,n);
...
*/
  ...
  qmgcv_qr(RU1,q,&qM,pivot,tau); /* RU1 and tau now contain the QR decomposition information */
  ...
  if (*type==0||*neg_w) { /* new Q factor needed explicitly */
    ...
    left=1;tp=0;qmgcv_qrqy(Qb,RU1,tau,q,&qM,&qM,&left,&tp); /* Q from the QR decomposition */
  ...
  if (*neg_w) { /* need to deal with -ve weight correction */
    ...
    bt=0;ct=0;qmgcv_mmult(IQQ,IQ,Qb,&bt,&ct,&k,&qM,q); /* I^-Q_1 \bar Q is k by rank */
    FREE(IQ);
     
    /* Get the SVD of IQQ */
    Vt = (double *)CALLOC((size_t) qM * qM,sizeof(double));
    d = (double *)CALLOC((size_t) qM,sizeof(double));
    qmgcv_svd_full(IQQ,Vt,d,&k,&qM); /* SVD of IQ */
    FREE(IQQ);
    for (i=0;i<qM;i++) {
      d[i] = 1 - 2*d[i]*d[i];
      if (d[i]<=0) d[i]=0.0; 
    ...
    /* Form P */
    bt=0;ct=1;qmgcv_mmult(P,Ri,Vt,&bt,&ct,&qM,&qM,&qM);

    /* Form K */
    if (*type==1) {
      bt=0;ct=0;qmgcv_mmult(K,XU1,P,&bt,&ct,n,&qM,&qM); 
    } else {
      work = (double *)CALLOC((size_t) *q * qM,sizeof(double));
      bt=0;ct=1;qmgcv_mmult(work,Qb,Vt,&bt,&ct,q,&qM,&qM); /* \bar Q V (I - 2D^2)^.5 */

      bt=0;ct=0;qmgcv_mmult(K,Q,work,&bt,&ct,n,&qM,q);
      FREE(work);
    }
    FREE(d);FREE(Vt);   
    ...
    if (*type==1) {
      ...
      bt=0;ct=0;qmgcv_mmult(K,Q,Qb,&bt,&ct,n,&qM,q);
    }
  }
...
{ int i,j,k,*pivot,nt1,nr,left,tp,bt,ct,TRUE=1,FALSE=0,one=1,rr;
  ...
  qmgcv_pqr(WX,n,q,pivot,tau,&nt1); /* WX and tau now contain the QR decomposition information */
  ...
  *n_drop = *q - *rank;
  ...
  qmgcv_qr(R,&nr,rank,pivot1,tau1); /* The final QR decomposition */ 
  ...
  if (neg_w || *type==0) { /* Q1 needed if neg_w correction needed, and anyway for type==0 */
    ...
    left=1;tp=0;qmgcv_qrqy(Q,R,tau1,&nr,rank,rank,&left,&tp); /* Q from the second QR decomposition */

    /* Q1 = Qb Q[1:q,]  where Qb from first QR decomposition... */
    for (i=0;i<rr;i++) for (j=0;j < *rank;j++) Q1[i + rr * j] = Q[i + nr * j];
    tp=0;qmgcv_pqrqy(Q1,WX,tau,n,&rr,rank,&tp,&nt1);
  ...
  if (neg_w) { /* then the correction for the negative w_i has to be evaluated */
    ...
    qmgcv_svd_full(IQ,Vt,d,&k,rank); /* SVD of IQ */
    FREE(IQ);
    ...
    /* form Rh = Vt R... */
    bt=0;ct=0;qmgcv_pmmult(Rh,Vt,P,&bt,&ct,rank,rank,rank,nt);
    ...
    /* Form P */
    bt=0;ct=1;qmgcv_pmmult(P,Ri,Vt,&bt,&ct,rank,rank,rank,nt);
    
    /* Form K - or postpone if type!=0*/
    if (*type==0) qmgcv_pmmult(K,Q1,Vt,&bt,&ct,n,rank,rank,nt);
  ...
  */
  ...
  if (*type==1) { /* create K = XP... */
    ...
    bt=1;ct=0;qmgcv_mmult(work,K,zz,&bt,&ct,rank,&one,n); /* K'z */ 
    qmgcv_mmult(work + *q *2,Q1,zz,&bt,&ct,rank,&one,n); /* Q1'z */
  }
  
  if (*type==1) { 
    bt=1;ct=0;qmgcv_mmult(work,K,z,&bt,&ct,rank,&one,n); /* K'Wz */
    applyP(PKtz,work,R,Vt,neg_w,nr,*rank,1,0);
  } else {
    /* Create Wz (not sqrt(|W|)z)... */
    for (i=0;i<*n;i++) zz[i] = raw[i] * raw[i] * z[i]; 
    for (i=0;i<neg_w;i++) { k=nind[i]; zz[k] = -zz[k];} 
    bt=1;ct=0;qmgcv_mmult(work + *q,X,zz,&bt,&ct,rank,&one,n); /* X'Wz */
...
{ double *work,*p0,*p1,*p2,*p3,*p4,*p5,*p6,*p7,*K=NULL,
  ...
  *nt = 1;
  ...
  if (*deriv) {
    ...
    if (*M>0) { 
      i=0;qmgcv_mmult(work,Rh,b1 + *n_theta * rank,&i,&i,&rank,M,&rank); /* Rh db/drho */
  ...
  //j=1; // debug &j for nt
  qmgcv_pqr(WX,&nr,&rank,pivot,tau,nt);
  ...
  tp=0;qmgcv_pqrqy(Q,WX,tau,&nr,&rank,&rank,&tp,nt);
...
{ double *WX,*tau,*work,*p0,*p1,*p2,*p3,*K=NULL,
  ...
  *nt = 1;
  ...
  if (*deriv) {
    ...
    bt=1;ct=0;qmgcv_mmult(dev_grad,X,v1,&bt,&ct,&rank,&one,n);
  ...
  if (*deriv) {
    ...
    if (*M>0) {
      i=0;qmgcv_mmult(work,Rh,b1,&i,&i,&rank,M,&rank); /* Rh db/drho */
    ...
    bt=1;ct=0;qmgcv_mmult(D1,b1,dev_grad,&bt,&ct,M,&one,&rank); /* gradient of deviance is complete */
  ...
  */ 
  ...
  if (*fisher) { /* then all quantites are the ones required for EDF calculations */ 
    ...
    tau = (double *)CALLOC((size_t)rank*(*nt+1),sizeof(double));
    /* st qmgcv_qr(WX,&nr,&rank,pivot,tau); */
    qmgcv_pqr(WX,&nr,&rank,pivot,tau,&nt1);
    ...
    /* st left=1;tp=0;qmgcv_qrqy(Q,WX,tau,&nr,&rank,&rank,&left,&tp); */ /* Q from the second QR decomposition */
    for (i=0;i< rank;i++) Q[i * rank + i] = 1.0;
    tp=0;qmgcv_pqrqy(Q,WX,tau,&nr,&rank,&rank,&tp,&nt1);
...
/* estimates the condition number of c by c upper triangular matrix 
   ...
   .C("R_cond",R=as.double(R),r=as.integer(m),c=as.integer(m),
       work=as.double(rep(0,4*m)),Rcond=as.double(1),PACKAGE="qmgcv")$Rcond
*/ 
...
{ int i,j,k,rank,one=1,*pivot,*pivot1,left,tp,neg_w=0,*nind,bt,ct,nr,rr,n_drop=0,*drop,TRUE=1,FALSE=0,nz;
  ...
  *nt = 1; /* no openMP support - turn off threading */
  ...
  /* st qmgcv_qr(WX,n,q,pivot,tau); */ /* WX and tau now contain the QR decomposition information */
  qmgcv_pqr(WX,n,q,pivot,tau,nt);
  ...
  */
  ...
  if (neg_w) { /* then the correction for the negative w_i has to be evaluated */
    ...
    /* st left=1;tp=0;qmgcv_qrqy(Q1,WX,tau,n,&rank,q,&left,&tp); */ /* Q1 = Qb Q[1:q,]  where Qb from first QR decomposition */   
    for (i=0;i<rr;i++) for (j=0;j<rank;j++) Q1[i + rr * j] = Q[i + nr * j];
    tp=0;qmgcv_pqrqy(Q1,WX,tau,n,&rr,&rank,&tp,nt);/* Q1 = Qb Q[1:rr,]  where Qb from first QR decomposition (contained in WX, tau) */   
  ...
  if (!*use_wy) { /* Now get the fitted values X \beta, *without* finding \beta */
    /* st left=1;tp=1;qmgcv_qrqy(z,WX,tau,n,&one,q,&left,&tp); */ /* z = Q'z */
    tp=1;qmgcv_pqrqy(z,WX,tau,n,&rr,&one,&tp,nt); 
    j = rank; //if (j > rr) j = rr; /* number of rows in r factor */
    for (i=j;i<nz;i++) z[i]=0.0;
    left=1,tp=1; 
    qmgcv_qrqy(z,R,tau1,&nr,&one,&rank,&left,&tp); /* z = Q1'Q'z, where Q1 is the first rank rows of second orth factor */
    ...
    if (neg_w) { /* apply the correction factor for negative w_i terms */
       bt=0;ct=0;qmgcv_mmult(work,Vt,z,&bt,&ct,&rank,&one,&rank); /* V' Q_1' z */
       for (i=0;i<rank;i++) work[i] *= d[i];
       bt=1;ct=0;qmgcv_mmult(z,Vt,work,&bt,&ct,&rank,&one,&rank); /* V (I-2D^2)^{-1} V' Q1' Q' z */
       for (i=0;i<rank;i++) y[i] = z[i];
    }
  
    left=1,tp=0; qmgcv_qrqy(z,R,tau1,&nr,&one,&rank,&left,&tp); /* z = Q1 Q1'Q'z */
    ...
    /* st left=1;tp=0;qmgcv_qrqy(z,WX,tau,n,&one,q,&left,&tp); */ /* z = Q Q1 Q1'Q z */
    tp=0;qmgcv_pqrqy(z,WX,tau,n,&rr,&one,&tp,nt); 

    for (i=0;i<*n;i++) eta[i] = z[i]/raw[i]; /* the linear predictor */
  } /* if (!*use_wy) */

  /* form X'Wz, drop any entries in drop and pivot... */
  bt=1;ct=0;qmgcv_mmult(work,X,wy,&bt,&ct,q,&one,n);     
  ...
  if (*use_wy) { /* re-compute other results from beta, as originals appear unstable or this method requested */
    ...
    bt=0;ct=0;qmgcv_mmult(eta,X,y,&bt,&ct,n,&one,q);
    bt=0;ct=0;qmgcv_mmult(work,E,y,&bt,&ct,rE,&one,q);
