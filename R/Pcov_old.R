#' Pcov_old
#' Pcov_old is meant for point process valued data,
#' an object of type 'ppp' or sf POINT objects can be handled as data input

#' Pcov_old can be used to process multiple covariates at the same time, each covariate will be given the name of the argument if provided,
#' or otherwise will simply be assigned the name of the symbol passed in
#' Pcov_old returns a representation of the point process containing the necessary components for setting up a convolutional basis representation.
...
#' a number of vctrs methods need to still be added for Pcov_olds, such as restore methods etc
...
Pcov_old <- function(..., resolution = c(128, 128), extent = NULL) {
    prepped <- lapply(list2(...), \(ob) pcov_prepare(ob, resolution, extent))
    vctrs::new_vctr(prepped,
                    class = c('Pcov_old', 'spatial_covariate'))
}

Pcov_old_impl <- function (points, extent) {
    
}

## Pcov_old should be evaluable when provided with a parametrically chosen kernel
#' @exportS3Method
evaluate.Pcov_old <- function(object, locations, kernel, ...){
